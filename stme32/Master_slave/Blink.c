// GVV Sharma
// September 5, 2018
// Blink two LEDs using Timer1 (master) and Timer2 (slave)
...
void setup() {
  ...
  GPIOA->CRL &= ~(GPIO_CRL_MODE2 | GPIO_CRL_CNF2); // Clear PA2 settings
  GPIOA->CRL |= (GPIO_CRL_MODE2_0 | GPIO_CRL_MODE2_1); // Output mode, max speed 50 MHz
  GPIOA->CRL &= ~GPIO_CRL_CNF2; // General purpose output push-pull

  // Configure Timer1 (Master)
  TIM1->SMCR = 0;             // Internal clock (8 MHz)
  TIM1->PSC = 7999;           // Prescaler (8 MHz / 8000 = 1 kHz)
  TIM1->ARR = 999;            // Auto-reload value (1 second)
  ...
  TIM2->ARR = 499;            // Auto-reload value (0.5 seconds)
...
void loop() {
  // Master LED (Timer1 -> PA1)
  if (TIM1->SR & 0x0001) {        // Check if Timer1 ARR count complete
    TIM1->SR &= ~0x0001;          // Clear Timer1 status register flag
    GPIOA->ODR ^= (1 << 1);       // Toggle PA1 (Master LED)
  }

  // Slave LED (Timer2 -> PA2)
  if (TIM2->SR & 0x0001) {        // Check if Timer2 ARR count complete
    TIM2->SR &= ~0x0001;          // Clear Timer2 status register flag
    GPIOA->ODR ^= (1 << 2);       // Toggle PA2 (Slave LED)
  }
}
